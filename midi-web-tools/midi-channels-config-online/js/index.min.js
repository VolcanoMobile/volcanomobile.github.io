var channelsConfig;var channelsConfigLayout;var channelsConfigSettingsLayout;var channelsConfigChannelSettingsLayout;
...
},receiveCallback:function(a){myMidiAccessApi.send(a)}});$(window).keypress(function(a){if(a.which===48){channelsConfig.setSelectedChannel(9)
...
});channelsConfig.setOnChannelCcValueChanged(function(c,a,b){channelsConfigChannelSettingsLayout.refreshCcLabels(a);
...
};this.construct(d)};var jqueryMusicMidiChannelsConfigChannelsLayout=function(f){var h={id:null,classePrefix:"jcc",channelsChannelMuteClass:"channels-channel-mute",notes:[36,38,39,50,47,43,42,46,51],notesText:(typeof gmStringDrumNotes==="undefined")?null:gmStringDrumNotes,notesTextShort:(typeof gmStringDrumNotesShort==="undefined")?null:gmStringDrumNotesShort,channelsConfig:null,onChannelClickCallback:null};
...
};var jqueryMusicMidiChannelsConfig=function(b){this.vars={numberOfChannels:16,channels:null,selectedChannel:0,enableControlChanges:[7,10,11],onNumberOfChannelsChanged:null,onSelectedChannelChanged:null,onEnableControlChangesChanged:null,onChannelNotesTypeChanged:null,onChannelProgramChanged:null,onChannelCcValueChanged:null};
...
this.vars.channels[d].fixDeserialize()}}};this.construct(b)};var jqueryMusicMidiChannelsConfigChannelCc=function(b){this.vars={value:0};var a=this;
...
}}};this.initProgramInput=function(){var i=c.rootElement.find(".programSelect");i.change(function(){if(h.onProgramChange!=null){h.onProgramChange(h.channelIndex,jQuery(this).val())
}});this.refreshProgramInputOptions()};this.refreshProgramInputOptions=function(){var k=h.channelsConfig.getChannel(h.channelIndex);
var l=c.rootElement.find(".programSelect");l.html("");for(var j=0;j<128;j++){var i=""+(j+1);
if(k!=null){if(k.getNotesType()==jqueryMusicMidiChannelsConfigChannel.NOTES_TYPES_DRUM){if(j<h.drumProgramsText.length){i+=" "+h.drumProgramsText[j]
}}else{i+=". "+h.programsText[j]}}else{console.log("refreshProgramInputOptions channelObject is NULL. vars.channelIndex: "+h.channelIndex+", "+h.channelsConfig.getChannelsCount())
}l.append(jQuery("<option/>").val(parseInt(j)).html(i))}this.refreshProgramInputValue()
};this.refreshProgramInputValue=function(){var i=h.channelsConfig.getChannel(h.channelIndex);
if(i){if(i.getProgramNumber()){this.rootElement.find(".programSelect").val(i.getProgramNumber())
...
};this.construct(e)};var jqueryMusicMidiChannelsConfigChannel=function(b){this.vars={programNumber:0,controlChanges:new Array(),notesType:jqueryMusicMidiChannelsConfigChannel.NOTES_TYPES_NOTE,changed:null};
...
jqueryMusicMidiChannelsConfigChannel.NOTES_TYPES_DRUM=1;