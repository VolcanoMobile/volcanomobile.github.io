...
},onPadEventChannelChange:function(f,b,a,c){var d=drumController.getPad(f,b);var e=d.getEvent(a);
if(e!=null){e.setChannel(c)}},onPadNoteValueChange:function(f,c,b,a){var d=drumController.getPad(f,c);
var e=d.getEvent(b);if(e!=null){e.setNoteValue(a)}drumControllerLayout.refreshPadState(f,c)
},onPadNoteVelocityChange:function(f,c,b,e){var d=drumController.getPad(f,c);var a=d.getEvent(b);
if(a!=null){a.setNoteVelocity(e)}}});myMidiAccessApi=new jMidiAccessApi({initCallback:function(a){jQuery("#midiNotSupported").css("display",a.midiSupported?"none":"block");
...
});$(window).keydown(function(b){var a=b.which||b.keyCode||0;if(a===69){togglePadSettingsPanel()
}else{if(a===77){jQuery("#midiPanel").slideToggle()}else{if(a===79){jQuery("#openFilePanel").slideToggle()
}else{if(a===83){jQuery("#controlerSettingsPanel").slideToggle()}else{if(a===97){handlePad(2,0,"noteOn")
}else{if(a===98){handlePad(2,1,"noteOn")}else{if(a===99){handlePad(2,2,"noteOn")}else{if(a===100){handlePad(1,0,"noteOn")
}else{if(a===101){handlePad(1,1,"noteOn")}else{if(a===102){handlePad(1,2,"noteOn")
}else{if(a===103){handlePad(0,0,"noteOn")}else{if(a===104){handlePad(0,1,"noteOn")
}else{if(a===105){handlePad(0,2,"noteOn")}}}}}}}}}}}}}});jQuery("#newProjectButton").click(function(){createDefaultProject()
...
}}}else{b.append("<button class='list-group-item'>No MIDI Access</button>")}}function listMidiOutputDevices(a){var b=jQuery("#midiOutputs");
b.html("");if(a!=null){if(a.outputs!=null){if(a.outputs.size>0){if(typeof a.outputs[Symbol.iterator]==="function"){a.outputs.forEach(function(e,d){var c="";
...
}else{myMidiAccessApi.connectOutputPortByName(f)}}))})}}else{b.append("<button class='list-group-item'>No Output available</button>")
}}}else{b.append("<button class='list-group-item'>No MIDI Access</button>")}}function download(a,c){var b=document.createElement("a");
