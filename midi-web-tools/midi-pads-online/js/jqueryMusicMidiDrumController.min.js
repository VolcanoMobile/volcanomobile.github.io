...
this.refreshInputsValues()};this.construct(d)};var jqueryMusicMidiDrumControllerPadSettingsLayout=function(d){var g={id:null,classePrefix:"jdc",rowIndex:0,columnIndex:0,notes:[36,38,39,50,47,43,42,46,51],drumNotesText:(typeof gmStringDrumNotes==="undefined")?null:gmStringDrumNotes,notesText:(typeof gmStringsNotes==="undefined")?null:gmStringsNotes,drumController:null,onAddPadEvent:null,onDeletePadEvent:null,onPadEventChannelChange:null,onPadNoteValueChange:null,onPadNoteVelocityChange:null};
var c=this;var a=null;var f=null;var e=null;var b=null;this.construct=function(h){$.extend(g,h);
if(g.drumNotesText==null){g.drumNotesText=new Array()}if(g.notesText==null){g.notesText=new Array()
...
}};this.initNoteLayout=function(r){var l=g.drumController.getPad(g.rowIndex,g.columnIndex);
var i=l.getEvent(r);if(i){var o=$("<select/>",{id:g.id+"-note-layout-note-channel-select-"+r,"class":g.classePrefix+"-note-channel-select form-control d-inline-block",style:" vertical-align: middle; max-width: 5rem;"}).change(function(){if(g.onPadEventChannelChange){g.onPadEventChannelChange(g.rowIndex,g.columnIndex,r,$(this).val())
}});for(var m=0;m<16;m++){o.append($("<option/>",{value:m}).html(m+1))}o.val(i.getChannel());
var q=$("<select/>",{id:g.id+"-note-layout-note-value-select-"+r,"class":g.classePrefix+"-note-value-select form-control d-inline-block",style:" vertical-align: middle; max-width: 10rem;"}).change(function(){if(g.onPadNoteValueChange){g.onPadNoteValueChange(g.rowIndex,g.columnIndex,r,$(this).val())
}});for(var s=0;s<128;s++){var k=parseInt(s/12);var n=g.notesText[s%12]+k;var h=g.drumNotesText[s];
if(!h){h=""}q.append($("<option/>",{value:s}).html(s+".  ("+n+") "+h))}q.val(i.getNoteValue());
var j=$("<input/>",{id:g.id+"-note-layout-note-velocity-range-"+r,"class":g.classePrefix+"-note-velocity-range",type:"range",min:0,max:127,value:i.getNoteVelocity(),style:"display: inline-block; vertical-align: middle;"}).change(function(){if(g.onPadNoteVelocityChange){g.onPadNoteVelocityChange(g.rowIndex,g.columnIndex,r,$(this).val())
}});var p=$("<button/>",{id:g.id+"-note-layout-note-velocity-range-"+r,"class":g.classePrefix+"-note-velocity-range btn float-right m-0",type:"range",min:0,max:127,value:i.getNoteVelocity(),style:" vertical-align: middle;"}).html("<span class='fa fa-times'></span>").click(function(){if(g.onDeletePadEvent){g.onDeletePadEvent(g.rowIndex,g.columnIndex,r)
}});this.noteLayout=$("<div/>",{id:g.id+"-note-layout","class":g.classePrefix+"-note-layout list-group-item p-0 px-2"}).append(p).append($("<span/>",{id:g.id+"-note-layout-note-index-"+r,"class":g.classePrefix+"-notes-index-layout",style:"display: inline-block; line-height: 4rem; vertical-align: middle; white-space: nowrap;"}).html((r+1)+". ")).append($("<span/>",{"class":"mx-2",style:"display: inline-block;  vertical-align: middle;"}).html("Channel ")).append(o).append($("<span/>",{"class":"mx-2",style:"display: inline-block; vertical-align: middle;"}).html(" Note ")).append(q).append($("<span/>",{"class":"mx-2",style:"display: inline-block; vertical-align: middle;"}).html(" Velocity ")).append(j).appendTo(this.notesLayout)
}};this.refreshPadValues=function(){var h=this.rootElement.find("#padSettingsInfoText");
var i=g.drumController.getPad(g.rowIndex,g.columnIndex);h.html("Pad "+(g.rowIndex+1)+"/"+(g.columnIndex+1))
...
};this.construct(d)};var jqueryMusicMidiDrumController=function(b){this.vars={maxRows:8,maxColumns:16,pads:null,rows:5,columns:6,callbackValuesChanged:null};
...
var u=l.getNoteValue();var r=(u<h.drumNotesText.length)?h.drumNotesText[u]:"X";if(!r){r=""
}var m="";if(l.getChannel()==9){m+=(u<h.drumNotesTextShort.length)?h.drumNotesTextShort[u]:"X"
}else{m+=gmStringsNotes[u%12]+parseInt(u/12)}if(!m){m="X"}j+=m;w+=r+"("+u+")"}var k=h.id+"-pads-row-"+x+"-cell-"+p;
var n=this.padsLayout.find("#"+k);n.attr("title",w);var t=h.classePrefix+"-pads-pad-label";
var s=n.find("."+t);s.html(j);var q=h.classePrefix+"-"+h.padsPadEditClass;var v=n.find("."+q);
v.css("display",h.editMode?"":"none")};this.getId=function(){return h.id};this.setDrumController=function(i){h.drumController=i;
this.createPadsRowsLayout()};this.getEditMode=function(){return h.editMode};this.setEditMode=function(i){h.editMode=i;
this.refreshPadsState()};this.construct(e)};var jqueryMusicMidiDrumControllerPadEvent=function(b){this.vars={type:jqueryMusicMidiDrumControllerPadEvent.TYPE_NOTE,channel:9,noteValue:36,noteVelocity:127,onChange:null};
...
};this.construct(b)};jqueryMusicMidiDrumControllerPadEvent.TYPE_NOTE=0;var jqueryMusicMidiDrumControllerPad=function(b){this.vars={events:[],changed:null};
...
this.vars.events[e]=Object.assign(c,d)}}}};this.construct(b)};