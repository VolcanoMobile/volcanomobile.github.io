...
if(a.midiSupported){devicesChanged(a)}},onRequestMidiAccessSuccess:function(a){if(a.midiSupported){devicesChanged(a)
...
});$(window).on("keydown",handleKeyPressed)});function handleKeyPressed(c){var a=c.key;
var b=c.which||c.keyCode||0;if(b===32){if(myMidiClock.isStarted()){myMidiClockPlayer.stop()
}else{myMidiClockPlayer.continue_()}if(c.stopImmediatePropagation){c.stopImmediatePropagation()
}if(c.stopPropagation){c.stopPropagation()}c.preventDefault()}else{if(b===13){if(myMidiClock.isStarted()){myMidiClockPlayer.stop()
}else{myMidiClockPlayer.start()}if(c.stopPropagation){c.stopPropagation()}c.preventDefault()
}else{if(a==="+"||b===107){myMidiClockPlayer.setBpm(myMidiClockPlayer.getBpm()+1)
}else{if(a==="-"||b===109){myMidiClockPlayer.setBpm(myMidiClockPlayer.getBpm()-1)
}else{if(b===68){$("#divisionCheckbox").click()}else{if(b===73){jQuery("#infoPanel").slideToggle()
}else{if(b===77){jQuery("#midiPanel").slideToggle()}else{if(b===83){jQuery("#timeSignaturePanel").slideToggle()
}else{if(b===84){$("#metronomeCheckbox").click()}}}}}}}}}}jQuery(document).ready(function(){jQuery("#inputLog").click(function(){jQuery(this).html("")
...
}function refreshBpmInfo(a){jQuery("#bpmValue").html(a+" BPM");jQuery("#bpmRange").val(a/2)
}function logText(a){jQuery("#inputLog").html(a)}function logMidiInputEventArray(g){var e="";
for(var b in g){var a=g[b];var d=0;for(var f in a){var c=a[f];if((typeof c)=="number"){c=(c+"").padStart(3,"0")
}else{c=c.padEnd(14,".")+""}e+=(d>0?", ":"")+f+": "+c;d++}}jQuery("#inputLog").prepend(e+"<br/>");
jQuery("#inputLog").html(jQuery("#inputLog").html().substring(0,2000))};